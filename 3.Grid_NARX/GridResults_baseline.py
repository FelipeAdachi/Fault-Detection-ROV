...
path='/home/felipeadachi/Dados/serverlab/Models_GridSearch/GYROZ/Baseline'
plt_index=['1','2','3']
subfolders=['ALL_MUTINFO','CFS_MUTINFO','RF_MUTINFO','SW_MUTINFO']

path_out='/home/felipeadachi/Dados/Scripts/Final_models/Baseline'
...
with open(path+'/log_narx.txt','r') as f:
    s=f.read()
lines=s.splitlines()
...
for line in lines:
    if line.startswith('Metricas'):
        ...
        df[st]=float(np.nanmean(mse_line))
        df_std[st]=float(np.nanstd(mse_line))
        
        df_mae[st]=float(np.nanmean(mae_line))
        df_mae_std[st]=float(np.nanstd(mae_line))
