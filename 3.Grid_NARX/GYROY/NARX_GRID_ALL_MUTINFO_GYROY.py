...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun  8 08:36:06 2019
...
train_file='/Train_'
val_file='/Validate_'
filename='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/GYROY/ALL_MUTINFO/log_narx.txt'
path_models='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/GYROY/ALL_MUTINFO'
...
def main():
    ...
    for combination in list(product(*param_grid.values())):
        ...
        for train_index, validate_index in kf.split(df):        
            ...
            y_train=df_train['GYROY'].values
            y_val=df_val['GYROY'].values
            ...
            x_train=df_train.drop(columns='GYROY').values
            x_val=df_val.drop(columns='GYROY').values
            ...
            print(x_train.shape)
            print(x_val.shape)
            dimension=int(len(eval(which)))

            
            models[i-1] = Sequential()
            models[i-1].add(Dense(dimension, input_dim=dimension, kernel_initializer='uniform', activation='tanh'))
        ...
        with open(filename,'a') as f:
            ...
            f.write("media mse>\n")
            f.write(str(np.nanmean(mse))+'\n')
            f.write("std mse>\n")
            f.write(str(np.nanstd(mse))+'\n')
            f.write('\n')
            
            f.write("media mae>\n")
            f.write(str(np.nanmean(mae))+'\n')
            f.write("std mse>\n")
            f.write(str(np.nanstd(mae))+'\n')
            f.write('\n')
