...
train_file='/Train_'
val_file='/Validate_'
filename='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/GYROX/CFS_MUTINFO/log_narx.txt'
path_models='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/GYROX/CFS_MUTINFO'
    ...
    for combination in list(product(*param_grid.values())):
        ...
        for train_index, validate_index in kf.split(df):        
            ...
            if i==1 or i==2:
                ...
                models[i-1].add(Dense(22, input_dim=22, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
            if i==3:
                models[i-1] = Sequential()
                models[i-1].add(Dense(20, input_dim=20, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))

            if i==4 or i==5:
                models[i-1] = Sequential()
                models[i-1].add(Dense(18, input_dim=18, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
