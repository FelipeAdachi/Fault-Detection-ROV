...
train_file='/Train_'
val_file='/Validate_'
filename='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/GYROX/ALL_MUTINFO/log_narx.txt'
path_models='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/GYROX/ALL_MUTINFO'
...
def main():
    ...
    for combination in list(product(*param_grid.values())):
        ...
        for train_index, validate_index in kf.split(df):        
            ...
            if i==3:
                models[i-1] = Sequential()
                models[i-1].add(Dense(46, input_dim=46, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))

            if i==4:
                models[i-1] = Sequential()
                models[i-1].add(Dense(44, input_dim=44, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))

            if i==5:
                models[i-1] = Sequential()
                models[i-1].add(Dense(59, input_dim=59, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
