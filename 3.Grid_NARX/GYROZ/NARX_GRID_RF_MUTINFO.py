...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 17:57:45 2019
...
train_file='/Train_'
val_file='/Validate_'
filename='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/RF_MUTINFO/log_narx.txt'
path_models='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/RF_MUTINFO'
...
def main():
    ...
    for combination in list(product(*param_grid.values())):
        ...
        for train_index, validate_index in kf.split(df):        
            ...
            print(x_train.shape)
            print(x_val.shape)
            #F1=26, F2=11, F3=27, F4=27,F5=11
            if i==1:
                models[i-1] = Sequential()
                models[i-1].add(Dense(26, input_dim=26, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
            if i==2 or i==5:
                models[i-1] = Sequential()
                models[i-1].add(Dense(11, input_dim=11, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
            
            if i==3 or i==4:
                models[i-1] = Sequential()
                models[i-1].add(Dense(27, input_dim=27, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
