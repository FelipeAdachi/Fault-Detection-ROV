...
train_file='/Train_'
val_file='/Validate_'
filename='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/CFS_MUTINFO/log_narx.txt'
path_models='/home/felipeadachi/Dados/Kfolds_cpu/Models_GridSearch/CFS_MUTINFO'
...
def main():
    ...
    for combination in list(product(*param_grid.values())):
        ...
        for train_index, validate_index in kf.split(df):        
            ...
            if i==1 or i==3 or i==4:
                ...
                models[i-1].add(Dense(13, input_dim=13, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
            else:
                models[i-1] = Sequential()
                models[i-1].add(Dense(17, input_dim=17, kernel_initializer='uniform', activation='tanh'))
                models[i-1].add(Dense(int(combination[0]), kernel_initializer='uniform', activation=str(combination[1])))
                models[i-1].add(Dense(1))
    ...
    with open(filename,'a') as f:
        print("Max mse is:",max(all_mse),file=f)
