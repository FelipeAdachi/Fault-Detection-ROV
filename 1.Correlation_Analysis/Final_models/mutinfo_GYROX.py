...
def main():
    ...
    for att in which_topast:
        ...
        #print("max correlation is>",corr['GYROX'][indx],"for timelag>",indx)
        timelag=indx.split("_")
        cross_corr.loc[att[0]]=[corr['GYROX'][indx],timelag[-1]]
    
        #print(corr['GYROX'])
        filename=''+att[0]+''
        corr['GYROX'].drop(index='GYROX').to_csv(path+'/Whole_train/mutinfo_analysis/GYROX/'+filename+'.csv')
        arr=corr['GYROX'].drop(index='GYROX').as_matrix()
        plt.plot(arr)
        plt.savefig(path+'/Whole_train/mutinfo_analysis/GYROX/'+filename+'.png')
        #plt.show() 
        #print("corr>",df3.corr())
    cross_corr.to_csv(path+'/Whole_train/mutinfo_analysis/GYROX/mutinfo_GYROX.csv')

    #print("valDF2>>>>>>>",valDF2)
    #print(validateDF)
    #y=trainDF['GYROX'].values
    #print(y)
    #print(y.shape)
    #y=y.reshape(-1,1)
...
def findmax(df):
    ...
    for index, row in df.iterrows():
        if abs(row['GYROX']) > maxcorr and abs(row['GYROX']) < 1: 
            maxcorr=abs(row['GYROX'])
            indx=index
    return maxcorr,indx        
