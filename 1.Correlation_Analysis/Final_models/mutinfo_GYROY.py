...
def main():
    ...
    for att in which_topast:
        ...
        #print("max correlation is>",corr['GYROY'][indx],"for timelag>",indx)
        timelag=indx.split("_")
        cross_corr.loc[att[0]]=[corr['GYROY'][indx],timelag[-1]]
    
        #print(corr['GYROY'])
        filename=''+att[0]+''
        corr['GYROY'].drop(index='GYROY').to_csv(path+'/Whole_train/mutinfo_analysis/GYROY/'+filename+'.csv')
        arr=corr['GYROY'].drop(index='GYROY').as_matrix()
        plt.plot(arr)
        plt.savefig(path+'/Whole_train/mutinfo_analysis/GYROY/'+filename+'.png')
        #plt.show() 
        #print("corr>",df3.corr())
    cross_corr.to_csv(path+'/Whole_train/mutinfo_analysis/GYROY/mutinfo_GYROY.csv')

    #print("valDF2>>>>>>>",valDF2)
    #print(validateDF)
    #y=trainDF['GYROY'].values
    #print(y)
    #print(y.shape)
    #y=y.reshape(-1,1)
