...
def main():
    ...
    for att in which_topast:
        ...
        cross_corr.loc[att[0]]=[corr['GYROZ'][indx],timelag[-1]]
    
        #print(corr['GYROZ'])
        filename=''+att[0]+''
        corr['GYROZ'].drop(index='GYROZ').to_csv(path+'/Whole_train/mutinfo_analysis/GYROZ/'+filename+'.csv')
        arr=corr['GYROZ'].drop(index='GYROZ').as_matrix()
        plt.plot(arr)
        plt.savefig(path+'/Whole_train/mutinfo_analysis/GYROZ/'+filename+'.png')
        #plt.show() 
        #print("corr>",df3.corr())
    cross_corr.to_csv(path+'/Whole_train/mutinfo_analysis/GYROZ/mutinfo_GYROZ.csv')
...
def findmax(df):
    ...
    for index, row in df.iterrows():
        if abs(row['GYROZ']) > maxcorr and abs(row['GYROZ']) < 1: 
            maxcorr=abs(row['GYROZ'])
            indx=index
    return maxcorr,indx        
