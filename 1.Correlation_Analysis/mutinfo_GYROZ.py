...
def main():
    ...
    for train_index, validate_index in kf.split(df):
        ...
        for att in which_topast:
            ...
            #print(corr['GYROZ'])
            filename=''+att[0]+''
            corr['GYROZ'].drop(index='GYROZ').to_csv(path+'/Fold_'+str(i)+'/mutinfo_analysis/GYROZ/'+filename+'.csv')
            arr=corr['GYROZ'].drop(index='GYROZ').as_matrix()
            plt.plot(arr)
            plt.savefig(path+'/Fold_'+str(i)+'/mutinfo_analysis/GYROZ/'+filename+'.png')
            #plt.show() 
            #print("corr>",df3.corr())
        cross_corr.to_csv(path+'/Fold_'+str(i)+'/mutinfo_analysis/GYROZ/mutinfo_GYROZ.csv')
