...
def main():
    ...
    for k in range(1,6):
        ...
        listdf=list(df)
        nstep=1
        with open(path+'/stepwise/GYROX.txt','a') as f:
        ...
        regressor_OLS = sm.OLS(endog = y, exog = x).fit()
        with open(path+'/stepwise/GYROX_OLS_Step_0.txt','a') as f:
            print(regressor_OLS.summary(),file=f)
        
        with open(path+'/stepwise/GYROX_OLS_Step_0_Latex.txt','a') as f:
        ...
        while max(regressor_OLS.pvalues[1:])>0.01:
            ...
            x=np.delete(x,[idx],1)
            regressor_OLS = sm.OLS(endog = y, exog = x).fit()
            with open(path+'/stepwise/GYROX_OLS_Step_'+str(nstep)+'.txt','a') as f:
                print(regressor_OLS.summary(),file=f)
        
            with open(path+'/stepwise/GYROX_OLS_Step_'+str(nstep)+'_Latex.txt','a') as f:
                print(regressor_OLS.summary().as_latex(),file=f)
            nstep+=1
            with open(path+'/stepwise/GYROX.txt','a') as f:
